...
public class ParsedAttestationRecord {
    ...
    private ParsedAttestationRecord(ASN1Sequence extensionData) {
        this.attestationVersion = ASN1Parsing.getIntegerFromAsn1(extensionData.getObjectAt(Constants.ATTESTATION_VERSION_INDEX));
        this.attestationSecurityLevel = securityLevelToEnum(
                ASN1Parsing.getIntegerFromAsn1(extensionData.getObjectAt(Constants.ATTESTATION_SECURITY_LEVEL_INDEX)));
        this.keymasterVersion = ASN1Parsing.getIntegerFromAsn1(extensionData.getObjectAt(Constants.KEYMASTER_VERSION_INDEX));
        this.keymasterSecurityLevel = securityLevelToEnum(
                ASN1Parsing.getIntegerFromAsn1(extensionData.getObjectAt(Constants.KEYMASTER_SECURITY_LEVEL_INDEX)));
        this.attestationChallenge = ((ASN1OctetString) extensionData.getObjectAt(Constants.ATTESTATION_CHALLENGE_INDEX))
                .getOctets();
        this.uniqueId = ((ASN1OctetString) extensionData.getObjectAt(Constants.UNIQUE_ID_INDEX)).getOctets();
        this.softwareEnforced = AuthorizationList.createAuthorizationList(
                ((ASN1Sequence) extensionData.getObjectAt(Constants.SW_ENFORCED_INDEX)).toArray(), attestationVersion);
        this.teeEnforced = AuthorizationList.createAuthorizationList(
                ((ASN1Sequence) extensionData.getObjectAt(Constants.TEE_ENFORCED_INDEX)).toArray(), attestationVersion);
    }
    ...
    private static SecurityLevel securityLevelToEnum(int securityLevel) {
        switch (securityLevel) {
            case Constants.KM_SECURITY_LEVEL_SOFTWARE:
                return SecurityLevel.SOFTWARE;
            case Constants.KM_SECURITY_LEVEL_TRUSTED_ENVIRONMENT:
                return SecurityLevel.TRUSTED_ENVIRONMENT;
            case Constants.KM_SECURITY_LEVEL_STRONG_BOX:
    ...
    private static ASN1Sequence extractAttestationSequence(X509Certificate attestationCert) throws IOException {
        byte[] attestationExtensionBytes = attestationCert.getExtensionValue(Constants.KEY_DESCRIPTION_OID);
    ...
    /**
     * This indicates the extent to which a software feature, such as a key pair, is
