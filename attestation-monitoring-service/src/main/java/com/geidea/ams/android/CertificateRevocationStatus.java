...
public class CertificateRevocationStatus {
    ...
    public static CertificateRevocationStatus loadStatusFromFile(BigInteger serialNumber, String filePath)
            throws IOException {
    ...
    private static CertificateRevocationStatus decodeStatus(String serialNumber, Reader statusListReader) {
        if (serialNumber == null) {
            throw new IllegalArgumentException("serialNumber cannot be null");
        }
        serialNumber = serialNumber.toLowerCase();

        JsonObject entries = new JsonParser().parse(statusListReader).getAsJsonObject().getAsJsonObject("entries");
