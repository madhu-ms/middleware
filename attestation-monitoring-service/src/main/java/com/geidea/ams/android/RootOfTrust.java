...
public class RootOfTrust {
    ...
    private RootOfTrust(ASN1Sequence rootOfTrust, int attestationVersion) {
        this.verifiedBootKey = ((ASN1OctetString) rootOfTrust.getObjectAt(Constants.ROOT_OF_TRUST_VERIFIED_BOOT_KEY_INDEX))
                .getOctets();
        this.deviceLocked = ASN1Parsing.getBooleanFromAsn1(rootOfTrust.getObjectAt(Constants.ROOT_OF_TRUST_DEVICE_LOCKED_INDEX));
        this.verifiedBootState = verifiedBootStateToEnum(
                ASN1Parsing.getIntegerFromAsn1(rootOfTrust.getObjectAt(Constants.ROOT_OF_TRUST_VERIFIED_BOOT_STATE_INDEX)));
        if (attestationVersion >= 3) {
            this.verifiedBootHash = ((ASN1OctetString) rootOfTrust.getObjectAt(Constants.ROOT_OF_TRUST_VERIFIED_BOOT_HASH_INDEX))
                    .getOctets();
        } else {
    ...
    private static VerifiedBootState verifiedBootStateToEnum(int securityLevel) {
        switch (securityLevel) {
            case Constants.KM_VERIFIED_BOOT_STATE_VERIFIED:
                return VerifiedBootState.VERIFIED;
            case Constants.KM_VERIFIED_BOOT_STATE_SELF_SIGNED:
                return VerifiedBootState.SELF_SIGNED;
            case Constants.KM_VERIFIED_BOOT_STATE_UNVERIFIED:
                return VerifiedBootState.UNVERIFIED;
            case Constants.KM_VERIFIED_BOOT_STATE_FAILED:
    ...
    /**
     * This provides the device's current boot state, which represents the level of
     * protection provided to the user and to apps after the device finishes
     * booting.
     */
